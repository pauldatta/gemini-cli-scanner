...
function extractSummary(manifest, filename) {
  ...
  // Defensive: privacy-mode manifests may have deleted sections (no user_prompts,
  // thought_topics, projects, tool_chains). Guard every access.
  const convos = manifest.conversations || {};
  const settings = manifest.settings || {};
  const policies = manifest.policies || {};
  const advisory = manifest.advisory || {};

  // Detect which AI CLIs are present
  const detected_tools = [];
  if (manifest.gemini_cli_installed !== false && (settings.found || manifest.gemini_cli_installed === true)) {
    detected_tools.push('Gemini CLI');
  }
  if (manifest.claude?.found) detected_tools.push('Claude Code');
  if (manifest.antigravity?.found) detected_tools.push('Antigravity');
  if (manifest.continue_dev?.found) detected_tools.push('Continue');
  if (manifest.windsurf?.found) detected_tools.push('Windsurf');
  if (manifest.jetbrains?.found) detected_tools.push('JetBrains AI');
  if (manifest.opencode?.found) detected_tools.push('OpenCode');
  ...
  return {
    ...
    maturity: advisory.maturity || { label: 'Unknown', emoji: '❓' },
    ...
    policy_count: (policies.parsed_rules || []).length,
    policy_has_deny_rm: (policies.parsed_rules || []).some(
      r => r.decision === 'deny' && (r.commandPrefix || '').includes('rm')
    ),
    policy_has_modes: (policies.parsed_rules || []).some(
      r => r.modes && r.modes.length > 0
    ),
    yolo_disabled: manifest._raw_settings?.security?.disableYoloMode || false,
    advisory_counts: advisory.summary || { critical: 0, warning: 0, info: 0 },
    tool_chains: (convos.tool_chains || [])
      .map(c => ((c.chain || c) instanceof Array ? c.chain : []).slice(0, 3).join('→'))
      .filter(Boolean),
    skills_count: (manifest.skills || []).length,
    mcp_count: Object.keys(settings.mcp_servers || {}).length,
    repos_count: manifest.repo_count || (manifest.repos || []).length,
    memory_tiers: manifest.memory_tiers?.summary?.tiers_in_use || 0,
    gemini_cli_installed: manifest.gemini_cli_installed !== false,
    detected_tools,
  };
}
...
function aggregateTeam(state) {
  ...
  return {
    ...
    tool_ecosystem: crossRefToolEcosystem(latestScans),
...
// ─── Tool Ecosystem Cross-Reference ──────────────────────────────────

function crossRefToolEcosystem(latestScans) {
  const TOOLS = ['Gemini CLI', 'Claude Code', 'Antigravity', 'Continue', 'Windsurf', 'JetBrains AI', 'OpenCode'];
  const toolMap = {}; // tool name → Set of reporter labels

  for (const { label, scan } of latestScans) {
    for (const tool of (scan.detected_tools || [])) {
      if (!toolMap[tool]) toolMap[tool] = new Set();
      toolMap[tool].add(label);
    }
  }
  ...
  return TOOLS.map(tool => ({
    name: tool,
    reporters: [...(toolMap[tool] || [])],
    count: (toolMap[tool] || new Set()).size,
    adoption: n > 0 ? (toolMap[tool] || new Set()).size / n : 0,
  })).filter(t => t.count > 0)
    .sort((a, b) => b.count - a.count);
}
