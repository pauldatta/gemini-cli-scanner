...
function computeScore(m) {
  ...
  if (m.antigravity?.found || m.antigravity_cli?.found || m.antigravity_ide?.found) ecosystemTools++;
  ...
  const rawTotal = Object.values(s).reduce((a, b) => a + b, 0);
  const RAW_MAX = 115;
  const normalized = Math.round((rawTotal / RAW_MAX) * 67);
  return { total: normalized, max: 67, breakdown: s };
}
...
function computeSkillOverlap(m) {
  ...
  addSkills(m.antigravity_cli?.skills);
  addSkills(m.antigravity_ide?.skills);
...
function generateReport(m) {
  const L = [];
  L.push('# AI Coding Environment Scan Report');
  L.push(`\n**Scan Date:** ${m.scan_timestamp}`);

  // Prominent banner when Gemini CLI is not installed
  if (m.gemini_cli_installed === false) {
    L.push('');
    L.push('> ⚠️ **Gemini CLI is not installed** — `~/.gemini` directory was not found.');
    L.push('>');
    L.push('> Install it: [google-gemini/gemini-cli](https://github.com/google-gemini/gemini-cli)');
    L.push('>');
    L.push('> This report contains partial results from other detected AI tools and scanned repos.');
    L.push('');
  }
  ...
  // AI Tool Ecosystem
  const ANTIGRAVITY_FLAVORS = [
    { key: 'antigravity_desktop', label: 'Antigravity Desktop', data: m.antigravity },
    { key: 'antigravity_cli', label: 'Antigravity CLI', data: m.antigravity_cli },
    { key: 'antigravity_ide', label: 'Antigravity IDE', data: m.antigravity_ide },
  ];
  ...
  if (ecosystemTools.length || (m.claude?.found)) {
    ...
    if (m.gemini_cli_installed === false) {
      L.push(`| Gemini CLI | ❌ Not installed | 0 | Install: [gemini-cli](https://github.com/google-gemini/gemini-cli) |`);
    } else {
    ...
    for (const t of ecosystemTools) {
      ...
      let details = '';
      if (t.key.startsWith('antigravity_')) {
        const parts = [];
        if (t.data.brain_conversations) parts.push(`${t.data.brain_conversations} brain sessions`);
        ...
        if (t.data.plugins?.length) parts.push(`${t.data.plugins.length} plugins`);
        if (t.data.history_entries) parts.push(`${t.data.history_entries} history entries`);
        details = parts.join(', ');
    ...
    // Brain intelligence — per-flavor
    for (const flav of ANTIGRAVITY_FLAVORS) {
      const brainI = flav.data?.brain_intelligence;
      if (!brainI || !brainI.total_steps) continue;
      L.push(`\n### 🧠 ${flav.label} — Brain Intelligence\n`);
      L.push(`- **Conversations parsed:** ${brainI.conversations_parsed} / ${flav.data.brain_conversations}`);
      ...
      if (Object.keys(brainI.tool_usage_top_20 || {}).length) {
        L.push('| Tool | Calls |', '|:---|---:|');
    ...
    const cli = m.antigravity_cli;
    if (cli?.found) {
      if (cli.plugins?.length) {
        L.push(`\n### 🔌 Antigravity CLI Plugins (${cli.plugins.length})\n`);
        for (const p of cli.plugins) L.push(`- **${p}**`);
      }
      if (cli.import_manifest) {
        const im = cli.import_manifest;
        L.push(`\n### 📦 Gemini CLI Migration\n`);
        L.push(`Imported ${im.imports?.length || 0} extensions from Gemini CLI.\n`);
        L.push('| Extension | Components |', '|:---|:---|');
        for (const imp of (im.imports || [])) {
          L.push(`| ${imp.name} | ${(imp.components || []).join(', ')} |`);
        }
      }
      if (cli.settings) {
        L.push(`\n### ⚙️ CLI Settings\n`);
        L.push(`- **Model:** ${cli.settings.model || 'default'}`);
        L.push(`- **Color scheme:** ${cli.settings.colorScheme || 'default'}`);
        L.push(`- **Trusted workspaces:** ${cli.settings.trustedWorkspaces || 0}`);
        L.push(`- **Permission rules:** ${cli.settings.permissionRules || 0}`);
      }
    }
  ...
  if (advisory && advisory.recommendations.length) {
    ...
    L.push(`\n## 🩺 Best Practices Advisory\n`);
    L.push(`**Maturity Rating:** ${mat.emoji || ''} **${mat.label || 'N/A'}** (${mat.score ?? '?'}/${mat.max ?? 67})\n`);
    L.push(`| Metric | Count |`, `|:---|---:|`);
    ...
    const CATEGORY_LABEL = {
      ...
      antigravity_migration: 'Antigravity Migration',
      antigravity_health: 'Antigravity Health',
    };
