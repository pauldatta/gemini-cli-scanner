...
function scanAntigravityFlavor(gdir, flavor = 'antigravity') {
  const agDir = path.join(gdir, flavor);
  if (!fs.existsSync(agDir)) return { found: false };
  const result = { found: true, flavor };

  // Skills
  result.skills = parseSkillsDir(path.join(agDir, 'skills'), flavor);

  // Brain conversations — parse transcript.jsonl for usage intelligence
  ...
  if (fs.existsSync(brainDir)) {
    ...
    // Deep parse: extract tool usage, prompts, and timespans from transcript.jsonl
    const toolUsage = {};
    const userPrompts = [];
    let totalSteps = 0;
    ...
    for (const cdir of convDirs) {
      // Prefer transcript.jsonl (present in all flavors), fall back to transcript_full.jsonl
      const logsDir = path.join(brainDir, cdir.name, '.system_generated', 'logs');
      const transcriptPath = path.join(logsDir, 'transcript.jsonl');
      const fullTranscriptPath = path.join(logsDir, 'transcript_full.jsonl');
      const content = tryRead(transcriptPath) || tryRead(fullTranscriptPath);
      if (!content) continue;
      conversationsParsed++;
  ...
  // Plugins (CLI and IDE have plugins/)
  const pluginsDir = path.join(agDir, 'plugins');
  if (fs.existsSync(pluginsDir)) {
    result.plugins = dirEntries(pluginsDir).filter(e => e.isDirectory()).map(e => e.name);
  }

  // CLI-specific data
  const settingsJson = tryReadJSON(path.join(agDir, 'settings.json'));
  if (settingsJson) {
    result.settings = {
      model: settingsJson.model || null,
      colorScheme: settingsJson.colorScheme || null,
      trustedWorkspaces: (settingsJson.trustedWorkspaces || []).length,
      permissionRules: (settingsJson.permissions?.allow || []).length,
    };
  }

  const historyPath = path.join(agDir, 'history.jsonl');
  if (fs.existsSync(historyPath)) {
    const histContent = tryRead(historyPath);
    result.history_entries = histContent ? histContent.split('\n').filter(l => l.trim()).length : 0;
  }

  const importManifest = tryReadJSON(path.join(agDir, 'import_manifest.json'));
  if (importManifest) {
    result.import_manifest = importManifest;
  }

  // Cache data (CLI-specific)
  const projectsCache = tryReadJSON(path.join(agDir, 'cache', 'projects.json'));
  if (projectsCache) {
    result.cached_projects = Object.keys(projectsCache).length;
  }

  return result;
}

/** Backward-compatible alias for scanning the Desktop flavor */
function scanAntigravity(gdir) {
  return scanAntigravityFlavor(gdir, 'antigravity');
}
...
module.exports = { scanSettings, scanGeminiMd, scanSkills, scanAgents, scanExtensions, scanPolicies, scanClaude, scanConversations, scanProjectGeminiMds, scanRepos, scanAntigravity, scanAntigravityFlavor, scanContinue, scanWindsurf, scanJetBrains, parseSkillsDir, scanMemoryTiers, scanSkillExtraction, scanAdminPolicies, scanOpenCode };
