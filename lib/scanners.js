...
function scanPolicies(gdir) {
  ...
  for (const f of dirEntries(pdir)) {
    if (f.name.endsWith('.toml')) {
      ...
      // Parse TOML into structured rules for the advisor engine
      const rules = parsePolicyToml(content);
      for (const r of rules) {
        if (r._table === 'rule') {
          parsed_rules.push({ filename: f.name, toolName: r.toolName, decision: r.decision, commandPrefix: r.commandPrefix, priority: r.priority });
        }
      }
    }
  }
