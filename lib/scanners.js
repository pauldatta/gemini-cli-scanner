...
function scanSkills(gdir) {
  const sdir = path.join(gdir, 'skills');
  const skills = [];
  for (const ent of dirEntries(sdir)) {
    ...
    const info = { name: ent.name, source: 'gemini', has_skill_md: hasMd };
...
function scanClaude(home) {
  ...
  const result = { found: true, skills: [], claude_mds: [] };
  const sdir = path.join(cdir, 'skills');
  for (const ent of dirEntries(sdir)) {
    if (!ent.isDirectory()) continue;
    const smPath = path.join(sdir, ent.name, 'SKILL.md');
    const hasMd = fs.existsSync(smPath);
    const info = { name: ent.name, source: 'claude', has_skill_md: hasMd };
    if (hasMd) Object.assign(info, parseFrontmatter(tryRead(smPath) || ''));
    result.skills.push(info);
  }
...
function scanConversations(gdir) {
...
module.exports = { scanSettings, scanGeminiMd, scanSkills, scanAgents, scanExtensions, scanPolicies, scanClaude, scanConversations, scanProjectGeminiMds, scanRepos };
