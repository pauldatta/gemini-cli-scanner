...
function aggregatePromptPatterns(prompts) {
  ...
  for (const p of prompts) {
    ...
    if (!clusters[fingerprint]) clusters[fingerprint] = { count: 0, examples: [], text: p.text, projects: new Set() };
    clusters[fingerprint].count++;
    if (p.project) clusters[fingerprint].projects.add(p.project);
  ...
  return Object.values(clusters)
    ...
    .slice(0, 15)
    .map(c => ({ count: c.count, examples: c.examples, project_count: c.projects.size }));
}

/**
 * Aggregate tool call chains into behavioral fingerprints.
 * Clusters sessions by the first 3 tools in their tool call sequence.
 */
function aggregateToolChains(chains) {
  if (!chains || !chains.length) return [];
  const clusters = {};
  for (const tc of chains) {
    const fp = (tc.chain || []).slice(0, 3).join('→');
    if (!fp) continue;
    if (!clusters[fp]) clusters[fp] = { count: 0, projects: new Set(), full_chains: [] };
    clusters[fp].count++;
    if (tc.project) clusters[fp].projects.add(tc.project);
    if (clusters[fp].full_chains.length < 3) clusters[fp].full_chains.push(tc.chain.slice(0, 8));
  }
  return Object.entries(clusters)
    .sort((a, b) => b[1].count - a[1].count)
    .slice(0, 10)
    .map(([fp, data]) => ({ fingerprint: fp, count: data.count, project_count: data.projects.size, examples: data.full_chains }));
}

/**
 * Soft evidence gate — checks if there's enough pattern repetition to justify
 * full skill writing (Pro model). Returns pass/fail with reasoning.
 * Mirrors the official confucius agent's evidence-based gating.
 */
function checkEvidenceThreshold(patterns, chainPatterns, prompts) {
  const totalPrompts = (prompts || []).length;
  // Gate 1: Need at least 10 qualifying prompts
  if (totalPrompts < 10) {
    return { pass: false, reason: `Only ${totalPrompts} prompts recorded (minimum 10 needed for pattern detection).` };
  }
  // Gate 2: At least one text pattern with ≥3 occurrences
  const strongTextPatterns = (patterns || []).filter(p => p.count >= 3);
  // Gate 3: At least one chain pattern with ≥3 occurrences
  const strongChainPatterns = (chainPatterns || []).filter(p => p.count >= 3);
  // Gate 4: Patterns span ≥2 projects
  const multiProjectText = strongTextPatterns.some(p => p.project_count >= 2);
  const multiProjectChain = strongChainPatterns.some(p => p.project_count >= 2);

  if (strongTextPatterns.length === 0 && strongChainPatterns.length === 0) {
    return { pass: false, reason: 'No prompt or tool-chain pattern found with ≥3 occurrences. Workflow is too varied for confident skill extraction.' };
  }
  if (!multiProjectText && !multiProjectChain) {
    return { pass: false, reason: 'Patterns found but confined to a single project. Cross-project repetition needed for reusable skills.' };
  }
  return {
    pass: true,
    reason: `${strongTextPatterns.length} text pattern(s) and ${strongChainPatterns.length} chain pattern(s) qualify.`,
    strong_text: strongTextPatterns.length,
    strong_chains: strongChainPatterns.length,
  };
}
...
function buildIdentifierPrompt(manifest) {
  ...
  const chainPatterns = aggregateToolChains(manifest.conversations?.tool_chains || []);
...
## Tool Chain Patterns (behavioral — what the user actually does)
${JSON.stringify(chainPatterns, null, 2)}

## Domain Topics (what the model thinks about most)
${JSON.stringify(Object.entries(convos.thought_topics_top_15 || {}).sort((a, b) => b[1] - a[1]).slice(0, 10))}
...
6. \`evidence_strength\` — "strong" (≥3 occurrences, ≥2 projects) or "moderate" (2+ occurrences)

Return valid JSON array with keys: name, description, rationale, key_tools, example_prompts, evidence_strength`;
}
...
async function suggestSkills(manifest, { apiKey, project } = {}) {
  ...
  // Evidence gate — check pattern strength before spending API tokens
  const convos = manifest.conversations || {};
  const allPrompts = [...(convos.user_prompts || [])];
  const agBrain = manifest.antigravity?.brain_intelligence;
  if (agBrain) allPrompts.push(...(agBrain.user_prompts || []));
  const patterns = aggregatePromptPatterns(allPrompts);
  const chainPatterns = aggregateToolChains(convos.tool_chains || []);
  const evidence = checkEvidenceThreshold(patterns, chainPatterns, allPrompts);

  if (!evidence.pass) {
    console.log(`  📊 Evidence gate: ${evidence.reason}`);
    console.log('  💡 Patterns found but not strong enough for confident skill generation.');
    console.log('     Continue using Gemini CLI — skills will be suggested when clear patterns emerge.');
    console.log('     Tip: Run /memory in the CLI to see what the extraction agent has found.');
    // Return pattern summary as soft suggestions
    return patterns.slice(0, 3).map(p => ({
      name: `pattern-${p.examples[0]?.split(/\s+/).slice(0, 3).join('-').toLowerCase().replace(/[^a-z0-9-]/g, '') || 'detected'}`,
      description: p.examples[0] || 'Detected repeating pattern',
      rationale: `Seen ${p.count} times across ${p.project_count || 1} project(s). Needs more repetition for confident extraction.`,
      evidence_strength: 'weak',
      skill_template: null,
    }));
  }

  console.log(`  📊 Evidence gate passed: ${evidence.reason}`);
  ...
  const skillPromises = candidates.map(async (candidate) => {
    ...
    return {
      ...
      evidence_strength: candidate.evidence_strength || 'strong',
...
module.exports = {
  ...
  _testing: { aggregatePromptPatterns, aggregateToolChains, checkEvidenceThreshold, buildRepoContext, buildIdentifierPrompt, buildSingleSkillWriterPrompt, extractJSON },
};
