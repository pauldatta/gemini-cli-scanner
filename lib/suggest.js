  ...
  for (const model of models) {
    try {
      ...
      if (msg.includes('404') || msg.includes('NOT_FOUND') || msg.includes('not found')) {
        if (!quiet) console.log(`    [${label}] ⚠ ${model} not available, trying next...`);
        continue;
      }
      if (!quiet) console.log(`    [${label}] ⚠ Failed (${model}): ${msg}`);
      return null;
    }
  }
  return null;
...
async function suggestSkills(manifest, { apiKey, project } = {}) {
  ...
  const skillPromises = candidates.map(async (candidate) => {
    ...
    return {
      ...
      skill_template: `---\nname: ${candidate.name}\ndescription: ${candidate.description}\n---\n# ${candidate.name}\n\nKey tools: ${(candidate.key_tools || []).join(', ')}\n\n_Skill template generation failed — write manually using the tools above._`,
    };
  });
